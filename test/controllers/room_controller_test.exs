defmodule Zchat.RoomControllerTest do
  use Zchat.ConnCase
  ...
  defp create_test_rooms(user) do
    ...
    Enum.each ["first room", "second room", "third room"], fn name ->
      Repo.insert! %Zchat.Room{owner_id: user.id, name: name}
    end

    # Create two rooms owned by another user
    other_user = Repo.insert! %Zchat.User{}
    Enum.each ["fourth room", "fifth room"], fn name ->
      Repo.insert! %Zchat.Room{owner_id: other_user.id, name: name}
    end
  end
